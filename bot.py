...
# Принудительно установить правильную версию PTB перед запуском
...
await query.edit_message_text(f”📚 *Грамматика — {level[‘name’]}*\n\nВыбери тему:”, reply_markup=build_markup(keyboard), parse_mode=“Markdown”)
...
reply_markup=build_markup(keyboard), parse_mode=“Markdown”)
return
ex = exercises[ex_idx]
keyboard = []
for i, opt in enumerate(ex[‘o’]):
keyboard.append([InlineKeyboardButton(f”{‘ABCD’[i]}. {opt}”, callback_data=f”g_ex_ans_{i}”)])
await query.edit_message_text(f”✏️ *Упражнение [{ex_idx + 1}/{len(exercises)}]*\n\n{ex[‘q’]}”, reply_markup=build_markup(keyboard), parse_mode=“Markdown”)
...
keyboard.append([InlineKeyboardButton(“◀️ Назад”, callback_data=“back_section”)])
await query.edit_message_text(f”📖 *Словарь — {level[‘name’]}*\n\nВыбери категорию:”, reply_markup=build_markup(keyboard), parse_mode=“Markdown”)
...
keyboard = [
[InlineKeyboardButton(“🔄 Повторить”, callback_data=f”v_cat*{cat_idx}”)],
[InlineKeyboardButton(“◀️ К категориям”, callback_data=“sec_vocab”)],
]
await query.edit_message_text(f”✅ *Категория завершена!*\n\nВы изучили все слова в «{cat[‘category’]}»!”, reply_markup=build_markup(keyboard), parse_mode=“Markdown”)
return
word = words[word_idx]
progress = f”{word_idx + 1}/{len(words)}”
keyboard = [
[InlineKeyboardButton(“➡️ Следующее” if word_idx + 1 < len(words) else “✅ Завершить”, callback_data=f”v_next_{word_idx + 1}”)],
[InlineKeyboardButton(“◀️ К категориям”, callback_data=“sec_vocab”)],
]
text = f”📖 *{cat[‘icon’]} {cat[‘category’]}* [{progress}]\n\n🇬🇧 *{word[‘en’]}*\n🇷🇺 {word[‘ru’]}\n\n💬 *{word.get(‘ex’, ‘’)}*”
await query.edit_message_text(text, reply_markup=build_markup(keyboard), parse_mode=“Markdown”)
...
keyboard.append([InlineKeyboardButton(“◀️ Назад”, callback_data=“back_section”)])
await query.edit_message_text(f”💬 *Фразы — {level[‘name’]}*\n\nВыбери ситуацию:”, reply_markup=build_markup(keyboard), parse_mode=“Markdown”)
